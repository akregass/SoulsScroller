...
public class DialogManager : MonoBehaviour {
	...
	void OnGUI (){
		if (isActive) {
			foreach (DialogCheck dC in godEye.questManager.dialogChecks) {
				if (!dC.completed) {
	...
	public void StartDialog (Actor actor, int id, int node){
		godEye.player.hasControl = false;
		currentActor = actor;
		SetDialog (id, node);
		isActive = true;
	}
	...
	public void CheckActions(DialogOption option){
		...
		if(option.questRef != null && option.questRef != "-1"){
			...
			QuestManager.StartQuest(godEye.questManager.questList[Int32.Parse (option.questRef)]);
		}

		if(option.incomingItems.Count > 0){
			foreach(KeyValuePair<int, int> item in option.incomingItems){
				godEye.player.inventory.AddItem (item.Key, item.Value);
			}
		}

		if(option.outgoingItems.Count > 0){
			foreach(KeyValuePair<int, int> item in option.outgoingItems){
				godEye.player.inventory.RemoveItem (item.Key, item.Value);
			}
		}
	}
